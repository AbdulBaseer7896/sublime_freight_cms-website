    ...
    def add_new_appointment_in_db(self , data , sale_man_pin):
        with self.engine.connect() as conn:
            ...
            if result[0][0] == None:
                ...
                print("This is 7")
                carear_id = int(result[0][0]) + 1
            query1 =  text(f"INSERT INTO new_appointment VALUES ( {carear_id} , '{data['company_name']}' ,'{data['usdot']}' , '{data['email']}' , '{data['truck_or_traler']}' , '{data['comments']}' , '{data['carear_name']}'  , '{data['mc']}'   , '{data['phone_number']}' , '{data['conversations']}' , '{data['appointment_type']}', {sale_man_pin} , '{data['date']}');")
            conn.execute(query1)
